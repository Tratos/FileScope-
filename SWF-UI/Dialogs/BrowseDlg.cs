...
{
	/// <summary>
	/// Choose from several hosts which to browse.
	/// </summary>
	public class BrowseDlg : System.Windows.Forms.Form
	{
		...
		public BrowseDlg()
		{
			...
			if(Stats.settings.alwaysOnTop)
				this.TopMost = true;
			toolTip1.SetToolTip(button1, "Browse with selected host(s)");
		}
		...
		private void InitializeComponent()
		{
			...
			this.label1.Text = "Browse-able Hosts:";
			// 
			// BrowseDlg
			// 
		...
		/// <summary>
		/// Add all of the browse compatible hosts from a QueryHitTable.
		/// </summary>
		...
		/// <summary>
		/// Add all of the browse compatible hosts from a download.
		/// </summary>
		public void SetupWindow(DownloadManager dmer)
		{
			foreach(Downloader dler in dmer.downloaders)
			{
				if(BrowseCompatible(dler.qho.vendor))
		...
		/// <summary>
		/// Add all of the browse compatible hosts from an upload.
		/// </summary>
		public void SetupWindow(Uploader uper)
		{
			if(BrowseCompatible(uper.userAgent))
		...
		/// <summary>
		/// Check if a vendor supports browsing.
		/// </summary>
		bool BrowseCompatible(string vendorUpper)
		{
			string vendor = vendorUpper.ToLower();
				if(vendor.IndexOf("filescope") != -1 || vendor.IndexOf("shareaza") != -1)
					return true;
			return false;
		}
		...
		private void button1_Click(object sender, System.EventArgs e)
		{
			foreach(object obj in listBox1.SelectedItems)
				StartApp.main.search.BrowseHost((string)obj);
			this.Close();
		}
	}
}
